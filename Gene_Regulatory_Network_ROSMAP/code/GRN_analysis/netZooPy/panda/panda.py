    ...
    def processData(
        ...
        if type(motif_file) is str:
            ...
            with Timer("Loading motif data ..."):
                self.motif_data = pd.read_csv(motif_file, sep="\t")
            ...
            with Timer("Loading expression data ..."):
                if with_header:
                    ...
                    self.expression_data = pd.read_csv(
                        expression_file, index_col = 0).set_index('symbol')
                    print(self.expression_data.cols)
                      

                else:
                    self.expression_data = pd.read_csv(
                        expression_file
                    , index_col = 0).set_index('symbol')
                ...
                #print(self.expression_genes)
                #print(self.expression_samples)
                print(self.expression_data.head())
                # self.num_genes = len(self.gene_names)
                # print('Expression matrix:', self.expression_data.shape)
        elif type(expression_file) is not str:
            # Pass expression as a dataframe se
        ...
        if (
            ...
            #print('Length of filtered Genes:', len(filtered_genes))
            #print(filtered_genes[0:10])
            #print(self.expression_data.loc[filtered_genes].values.shape)
